...
public class IndexController extends Controller 
{
    ...
    */
    public Result showCatalog(long id)
    {
    	...
    	boolean ratingStamp = false;
    	boolean privateStamp = false;
    	String value = "";
        ...
        if(token!= null)
        {
            ...
            if(tempUser.user_type == 2)
            {
            	...
            	if(!ratingStamp && !privateStamp )
            	{
            		return ok(catalogArtist.render(listStamp,token,userName,tempOptions,"catalog"));
            	}
            	else if(!ratingStamp && privateStamp)
            	{
            		return ok(catalogArtistPrivate.render(listStamp,token,userName,tempOptions,"catalog"));
            	}
            	else if(ratingStamp && !privateStamp)
            	{
            		return ok(catalogArtistRating.render(listStamp,token,userName,tempOptions,"catalog"));
            	}
            	else
            	{
            		return ok(catalogArtistPrivateRating.render(listStamp,token,userName,tempOptions,"catalog"));
            	}
            ...
            else
            {
            	...
            	if(!ratingStamp && !privateStamp )
            	{
            		return ok(catalogAdmin.render(listStamp,token,userName,tempOptions,"catalog"));
            	}
            	else if(!ratingStamp && privateStamp)
            	{
            		return ok(catalogAdminPrivate.render(listStamp,token,userName,tempOptions,"catalog"));
            	}
            	else if(ratingStamp && !privateStamp)
            	{
            		return ok(catalogAdminRating.render(listStamp,token,userName,tempOptions,"catalog"));
            	}
            	else
            	{
            		return ok(catalogAdminPrivateRating.render(listStamp,token,userName,tempOptions,"catalog"));
            	}
    ...
    public Result viewStampRating(long id)
    {
        String token = session("token");
        String userId = session("user_id");
        String userName = null;
        ...
        if(id != 0)
        {
            ...
            if(tempStamp !=null)
            {
                ...
                ArtistProfile tempArtistProfile = ArtistProfile.find.where().eq("user_id",tempArtist).findUnique();
                session("stamp_id", ""+tempStamp.stamp_id);
                List<StampRating> ratings = StampRating.find.where().eq("stamp_id", tempStamp).eq("rating_status", 1).findList();
                return ok(viewStampRating.render(listCategories,ratings,tempStamp,tempArtist,tempArtistProfile,tempOptionsStamp,tempOptionsArtist,
                token,userName,tempOptions,"catalog"));
            }
            else
            {
    ...
    public Result createPrivateStamp()
    {
        ...
        if(token != null)
        {
            ...
            if(tempUser.user_type == 2)
            {
                ...
                return ok(create_stamp_private.render(tempForm,null,token,tempUser.user_first_name,tempOptions,categoriesNames,"createPrivateStamp"));
            }
            else
            {
                return redirect("/");
